...
async function getProductsSafe() {
	...
	try {
        ...
        // 			{
        //   "status": false,
        //   "message": "請確認 API Path 是否已申請 (っ˘ω˘ς )"
        // }
...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   ...
   - 4xx：客戶端錯誤，表示請求有誤或無法處理
   - 5xx：伺服器錯誤，表示伺服器在處理請求時發生錯誤

2. GET、POST、PATCH、PUT、DELETE 的差異
   答：
   - GET：用於取得資源，不應該有副作用（安全且可快取）
   - POST：用於建立資源，會有副作用（不安全且不可快取）
   - PATCH：用於部分更新資源，會有副作用（不安全且不可快取）
   - PUT：用於完全更新資源，會有副作用（不安全且不可快取）
   - DELETE：用於刪除資源，會有副作用（不安全且不可快取）

3. 什麼是 RESTful API？
   答：
   RESTful API 是一種遵循 REST（Representational State Transfer）架構原則的 API 設計方式。
   它使用標準的 HTTP 方法（GET、POST、PATCH、PUT、DELETE）來操作資源，並通過 URL 來識別資源的位置。
   RESTful API 的設計目標是使 API 更加簡潔、易於理解和使用。
...
if (require.main === module) {
	async function runTests() {
		...
		// try {
		// 	const result = await addToCart("zSCGKMReM2apAy0WgFCQ", 3);
        ...
        //   try {
		// 	const result = await updateCartItem("CIqGGSUOItLr703ocafB", 1);
		...
		//     try {
		// 	const result = await removeCartItem("CIqGGSUOItLr703ocafB");
